...
router.get('/eliminarLibro/:id', async(req,res) =>{
 ...
 router.post('/carrito', async (req,res) =>{
    ...
    try {
        ...
        carrito.forEach(async element => {
            contador +=1;
            await pool.query("Insert into detalle_boleta (ID_PEDIDO,ID_LIB,PRECIO_UNITARIO,CANTIDAD,TOTAL)VALUES ('"+id+"','"+element.ID+"','"+element.PRECIO+"','"+req.body.CANTIDAD[contador-1]+"','" + (element.PRECIO * req.body.CANTIDAD[contador-1]) +"')");
        ...
        let style = ( 'html,body {'+
            ...
            carrito.forEach(async element => {
                ...
                mensaje += (
                    ...
                    '<td valign="middle" width="10%" style="text-align:center; padding: 0 2.5em;">' + req.body.CANTIDAD[contador-1] +'</td>' +
                    '<td valign="middle" width="20%" style="text-align:center; padding: 0 2.5em;">' + (element.PRECIO * req.body.CANTIDAD[contador-1]) +'</td></tr>');
                    total = total+ (element.PRECIO * req.body.CANTIDAD[contador-1]);
